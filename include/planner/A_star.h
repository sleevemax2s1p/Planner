...
{
    if (target->x < 0 || target->y < 0 || target->z < 0 || target->x >= map.size() || target->y >= map[0].size() || target->z >= map[0][0].size() || map[target->x][target->y][target->z] == 1 || (point->x == target->x && point->y == target->y && point->z == target->z) || isInList(closelist, target))
        return false;
    else
        return true;
}
    ...
    for (int i = point->x - 1; i <= point->x + 1; i++)
        for (int j = point->y - 1; j <= point->y + 1; j++)
            for (int k = point->z - 1; k <= point->z + 1; k++)
            {
                ...
                if (isCanreach(point, surround_point))
                    surround_points.push_back(surround_point);
                else
                    delete surround_point;
            }
