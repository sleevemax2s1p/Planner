...
template <int Dim>
vec_Vecf<Dim> JPSPlanner<Dim>::getForce() { return force_; }
...
bool JPSPlanner<Dim>::plan(const Vecf<Dim> &start, const Vecf<Dim> &goal, decimal_t eps, bool use_jps)
{
  ...
  status_ = 0;
  /////********************************************************************************************************
  // std::cout <<"start111111: " << start.transpose() << std::endl;
  const Veci<Dim> start_int = map_util_->floatToInt(start);
  //  std::cout << "start_int11111: " << start_int.transpose() << std::endl;
  //  std::cout <<"goal11111:  " << goal.transpose() << std::endl;
  //   const Veci<Dim> goal_int1 = map_util_->floatToInt(goal);
  //    std::cout <<"goal_int1111:  " << goal.transpose() << std::endl;
  //**************************************************************************************************************
  ...
  force_.clear();
  const auto force = graph_search_->getForce();
  if (use_jps && Dim == 2)
  {
    for (const auto &it : force)
    {
      force_.push_back(map_util_->intToFloat(Veci<Dim>(it->x, it->y)));
    }
    std::reverse(std::begin(force_), std::end(force_));
  }
