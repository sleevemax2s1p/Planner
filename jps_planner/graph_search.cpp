    ...
    for( int s = 0; s < (int) succ_ids.size(); s++ )
    {
      ...
      if( tentative_gval < child_ptr->g )
      {
        ...
        if( child_ptr->opened && !child_ptr->closed) {
          ...
          // if(use_jps_)
          // {
          //   std::cout<<child_ptr->dx<<"   "<<child_ptr->dy<<"   "<<child_ptr->dz<<std::endl;
          // }
          if(child_ptr->dx != 0)
...
std::vector<StatePtr> GraphSearch::recoverPath(StatePtr node, int start_id) {
  ...
  while(node && node->id != start_id) {
    ...
    if(use_jps_)
    {
      if(node->dx!=dxy[0]||node->dy!=dxy[1])
      {
        force_.emplace_back(node);
      }
    dxy[0] = node->dx;
    dxy[1] = node->dy;
    }
...
std::vector<StatePtr> GraphSearch::getForce() const {
  return force_;
}
